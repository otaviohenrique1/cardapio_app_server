...
export default {
  ...
  async show(request: Request, response: Response, next: NextFunction) {
    ...
    const pedidoRepository = getRepository(Pedido);
    const pedido = await pedidoRepository.findOneOrFail(codigo);
  ...
  async create(request: Request, response: Response, next: NextFunction) {
    const { nome, status_pedido, lista_refeicoes, preco_pedido, data_cadastro, data_modificacao_cadastro } = request.body;
    const pedidoRepository = getRepository(Pedido);
    const data = { nome, status_pedido, lista_refeicoes, preco_pedido, data_cadastro, data_modificacao_cadastro };
  ...
  async update(request: Request, response: Response, next: NextFunction) {
    const { codigo, status_pedido, data_modificacao_cadastro } = request.body;
    const pedidoRepository = getRepository(Pedido);
    const data = { status_pedido, data_modificacao_cadastro };
    const schema = Yup.object().shape({
      status_pedido: Yup.string().required(MensagemCampoVazio('status_pedido')),
      data_modificacao_cadastro: Yup.date().required(MensagemCampoVazio('data_modificacao_cadastro')),
    });
    await schema.validate(data, { abortEarly: false });
