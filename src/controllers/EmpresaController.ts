...
import { valida_data_cadastro, valida_data_modificacao_cadastro, valida_email, valida_nome, valida_senha, valida_status_cadastro } from "../utils/SchemasValidacao";
...
export default {
  ...
  async login(request: Request, response: Response, next: NextFunction) {
    ...
    try {
      existingUser = await empresaRepository
        .findOne({ email: email });
    } catch (error) {
      return response
        .status(500)
